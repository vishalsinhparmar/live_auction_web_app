...
import { startAuctionDataAsync } from "../../features/auction/auctionSlice";
import { toast } from "react-toastify";

const UserItem = () => {
  const dispatch = useDispatch();
  const { loading, message, error, auctionItemData } = useSelector((state) => state.userItem);
  ...
  const handleClick = async (id) => {
    ...
    if (message?.userAuctionItemAsync) {
      toast.success("Auction started successfully!");
      dispatch(userAuctionItemAsync()); // Refresh list
    } else{
      toast.error(error?.userAuctionItemAsync || "Something went wrong.");
    }
  };
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {auctionItemData?.map((item) => (
          <div
            key={item._id}
            className="border rounded-lg shadow-md p-4 space-y-4 bg-white"
          >
